...
for depth in range(1,20,1):
    ...
    model_DTC.fit(features_train, target_train).score(features_valid, target_valid)
...
for est in range(10, 51, 10):
    for depth in range(1, 12):
        ...
        if f1_res<res:
            ...
            est_model=est
            depth_model=depth
        print('estim',est,'depth',depth,'F1:',f1_score(target_valid,model_RFC_prediction))
print('--------------------------------')
...
for depth in range(1,20,1):
   ...
   model_DTC_upsampled_prediction = model_DTC_upsampled.predict(features_valid)
   ...
   if f1_res<res:
       ...
       depth_model=depth
print('Лучший результат upsampled - depth',depth_model,'F1:',f1_score(target_valid,p))


f1_res=0
for depth in range(1,20,1):
   model_DTC_downsampled=DecisionTreeClassifier(max_depth=depth,random_state=12345)
   model_DTC_downsampled.fit(features_downsampled, target_downsampled)
   model_DTC_downsampled_prediction = model_DTC_downsampled.predict(features_valid)
   ...
   if f1_res<res:
       ...
       depth_model=depth
print('Лучший результат downsampled - depth',depth_model,'F1:',f1_score(target_valid,p))
...
for est in range(10, 51, 10):
    for depth in range(1, 12):
        model_RFC_upsampled = RandomForestClassifier( n_estimators=est, max_depth=depth,random_state=12345)
        model_RFC_upsampled.fit(features_downsampled, target_downsampled)
        model_RFC_upsampled_prediction = model_RFC_upsampled.predict(features_valid)
...
for est in range(10, 51, 10):
    for depth in range(1, 12):
        model_RFC_downsampled = RandomForestClassifier( n_estimators=est, max_depth=depth,random_state=12345)
...
print('Лучший результат upsampled - ', f1(target_valid, LG_prediction_upsampled))
print()
model_LG_downsampled = LogisticRegression(solver = 'liblinear', class_weight='balanced')
...
print('Лучший результат downsampled - ', f1(target_valid, LG_prediction_downsampled))
